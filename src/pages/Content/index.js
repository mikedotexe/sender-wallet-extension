...
window.addEventListener('message', async function (event) {
  try {
    ...
    if (request.type === 'sender-wallet-fromPage' && request.method === 'getRpc') {
      window.postMessage({ ...request, type: 'sender-wallet-result', rpc: currentRpc[network] });
    }
    ...
    if (key === 'persist:root') {
      ...
      updatePersistStore().then((res) => {
        ...
        if (oldPersistStore.app.currentAccount.accountId !== res.app.currentAccount.accountId) {
          ...
          window.postMessage({ type: 'sender-wallet-fromContent', method: 'rpcChanged', rpc: res.app.currentRpc[network] });
        }
